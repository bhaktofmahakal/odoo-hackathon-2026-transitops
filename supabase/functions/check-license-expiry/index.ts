...
serve(async (req) => {
  ...
  try {
    ...
    if (officersError) {
      throw new Error(
        `Failed to query safety officers: ${officersError.message}`,
      );
    }
    ...
    if (drivers && drivers.length > 0 && officers && officers.length > 0) {
      const expiringList = drivers
        .map(
          (d) =>
            `• ${d.name} (License #${d.license_number}) expires on ${d.license_expiry_date}`,
        )
        .join("\n");
      ...
      if (RESEND_API_KEY) {
        ...
        if (!emailResponse.ok) {
          ...
          console.log(
            `Alert email successfully sent to: ${recipients.join(", ")}`,
          );
        }
      } else {
        console.warn(
          "RESEND_API_KEY not configured. Skipping email delivery, in-app notification only.",
        );
      }
    }
